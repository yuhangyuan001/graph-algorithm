...
$(function () {
  ...
  $(".btn").click(function () {
    ...
    $(this)
      .parents(".arithmeticList")
      .siblings(".arithmeticList")
      .children(".buttonList")
      .children()
      .css("background", "");
  ...
  */
  ...
  $(".ThrdBtreeCreateLine").click(function () {
    ...
    var childNode = $(".childNodeDetail").val();
    if (canThrd == 1) {
      currentThrdBTree.createButtonCallBack(
        parentNode,
        nodePosition,
        childNode
      );
  ...
  $("#sendButton").click(async () => {
    ...
    // 显示用户消息，使用 append() 方法追加消息，替换文本为头像
    messagesDiv.append(
      `<div class="message user-message">
         <span>${userMessage}
                  <p>${new Date().toLocaleString()}</p></span>
         <img src="../image/user-avatar.png" alt="用户头像" class="user-avatar" />
       </div>`
    );
    console.log(messagesDiv.html());
    userInput.val("");
    ...
    try {
      ...
      messagesDiv.append(
        `<div class="message ai-message">
         <img src="../image/ai-avatar.png" alt="AI头像" class="ai-avatar" />
         <span>${data.content}
         <p>${data.timestamp}</p></span>
         
       </div>`
      );
      messagesDiv.scrollTop = messagesDiv[0].scrollHeight; // 滚动到最新消息
    } catch (error) {
